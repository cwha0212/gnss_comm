...
namespace gnss_comm
{
    static void set_header_stamp(std_msgs::msg::Header & header, double sec)
    {
        header.stamp.sec = static_cast<int32_t>(std::floor(sec));
        header.stamp.nanosec = static_cast<uint32_t>((sec - std::floor(sec)) * 1e9);
    }
    ...
    gnss_comm::msg::GnssBestXYZMsg best_xyz2msg(const BestXYZPtr &best_xyz)
    {
        ...
        set_header_stamp(gnss_best_xyz_msg.header, time2sec(best_xyz->time));
    ...
    gnss_comm::msg::GnssSvsMsg svs2msg(const std::vector<SvInfo> &svs)
    {
        ...
        set_header_stamp(svs_msg.header, time2sec(svs[0].time));
        for (auto & sv : svs)
        {
